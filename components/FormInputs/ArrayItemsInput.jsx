...
export default function ArrayItemsInput({setItems, items, itemTitle}) {
  ...
  <button
        ...
        type='button'
        className='flex items-center space-x-2 text-slate-900 dark:text-slate-300 py-2 px-4'
      >
        <Plus/>
      ...
      items.map((item, i)=>{
        return(
          <div 
            ...
            key={i} 
            className="flex cursor-pointer space-x-2 items-center bg-slate-200 dark:bg-slate-600 py-2 px-4 rounded-lg dark:text-slate-100 text-slate-900">
            <p>{item}</p>
              <X className='w-4 h-4'/>
          </div>
        )
